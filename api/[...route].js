...
authRoutes.post("/login", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Senha obrigat\xF3ria" }, 400);
  }
...
var PostgresJsPreparedQuery = class extends PgPreparedQuery {
  constructor(client2, queryString, params, logger2, cache2, queryMetadata, cacheConfig, fields, _isResponseInArrayMode, customResultMapper) {
...
__export(schema_exports, {
  ...
  flowSteps: () => flowSteps,
  leads: () => leads,
  mensagensAgendadas: () => mensagensAgendadas,
...
var PRODUCT_TYPES = ["saas", "curso", "digital", "indefinido"];
var produtos = pgTable("produtos", {
  id: serial("id").primaryKey(),
  nome: text("nome").notNull().unique(),
  tipo: text("tipo").$type().notNull().default("indefinido"),
  cor: text("cor"),
  gatewayMatch: jsonb("gateway_match").$type().notNull().default([]),
...
var leads = pgTable("leads", {
  ...
  status: text("status").$type().notNull().default("lead_novo"),
  origem: text("origem").$type(),
  ...
  // Assinatura
  subscriptionStatus: text("subscription_status").$type().notNull().default("nenhuma"),
...
var mensagensAgendadas = pgTable("mensagens_agendadas", {
  id: serial("id").primaryKey(),
  leadId: integer("lead_id").notNull().references(() => leads.id, { onDelete: "cascade" }),
  template: text("template").$type().notNull(),
...
var messageTemplates = pgTable("message_templates", {
  id: serial("id").primaryKey(),
  key: text("key").$type().notNull().unique(),
  nome: text("nome").notNull(),
  descricao: text("descricao"),
  conteudo: text("conteudo").notNull(),
  conteudoDefault: text("conteudo_default").notNull(),
  placeholdersDisponiveis: jsonb("placeholders_disponiveis").$type().notNull().default([]),
  atualizadoEm: timestamp("atualizado_em", { mode: "date" }).notNull().defaultNow(),
  criadoEm: timestamp("criado_em", { mode: "date" }).notNull().defaultNow()
});
var flowSteps = pgTable("flow_steps", {
  id: serial("id").primaryKey(),
  gatewayEvent: text("gateway_event").notNull(),
  ordem: integer("ordem").notNull(),
  templateKey: text("template_key").$type().notNull(),
...
function detectEventType(payload) {
  const status = String(pick(payload, "status") ?? "").toLowerCase().trim();
  const method = String(pick(payload, "payment_method") ?? "").toLowerCase().trim();
  const subStatus = String(pick(payload, "subscription.status") ?? "").toLowerCase().trim();
  if (subStatus) {
  ...
  if (status === "subscription_delayed") {
    const successful = pick(payload, "subscriptions.0.successful_charges");
  ...
  const probe = `${status} ${pick(payload, "event", "event_name", "type") ?? ""}`.toLowerCase();
...
function parsePhone(raw2) {
  ...
  const digits = String(raw2).replace(/\D/g, "");
  if (!digits) return null;
  if (digits.length === 10 || digits.length === 11) return `55${digits}`;
  return digits;
}
function parseTictoAmount(raw2) {
  if (raw2 == null) return null;
  if (typeof raw2 === "number" && Number.isFinite(raw2)) return raw2 / 100;
  if (typeof raw2 === "string") {
    const cleaned = raw2.replace(/[R$\s]/g, "").replace(/\./g, "").replace(",", ".");
    const n = Number(cleaned);
    return isNaN(n) ? null : n;
  }
  return null;
}
function parseDate(raw2) {
  if (!raw2) return null;
  const d = new Date(raw2);
  return isNaN(d.getTime()) ? null : d;
}
function parseTictoWebhook(payload) {
  const eventType = detectEventType(payload);
  if (!eventType) return null;
  const nome = pick(payload, "customer.name", "buyer.name", "client.name", "name") ?? "Cliente Ticto";
  const email = pick(payload, "customer.email", "buyer.email", "client.email", "email") ?? null;
  const phoneRaw = pick(
    payload,
    "customer.phone",
    "customer.whatsapp",
  ...
  const pixExpiraEm = parseDate(pick(payload, "pix.expires_at", "pix_expires_at", "expires_at")) ?? null;
  return {
    source: "ticto",
    eventType,
    rawPayload: payload,
    ...
    extras: {
      ...
      link_checkout: pick(payload, "checkout_url", "order.checkout_url") ?? "",
      link_pix: pick(payload, "pix.qrcode", "pix_qrcode") ?? "",
      link_boleto: pick(payload, "transaction.bank_slip_url", "bank_slip_url") ?? "",
      // change_card_url: link unico pra cliente retomar pagamento da assinatura sem refazer checkout
      change_card_url: pick(payload, "subscriptions.0.change_card_url") ?? ""
    }
  };
}
function verifyTictoSignature(payload) {
  const secret = process.env.TICTO_WEBHOOK_SECRET;
  if (!secret) return { valid: true, reason: "no-secret-configured" };
  const received = pick(payload, "token", "security_token", "webhook_token");
  if (!received) return { valid: false, reason: "missing-token-in-payload" };
  const a = Buffer.from(String(received));
  const b2 = Buffer.from(secret);
  if (a.length !== b2.length) return { valid: false, reason: "token-mismatch" };
  let diff = 0;
...
function parseStripeAmount(raw2) {
  if (typeof raw2 !== "number" || !Number.isFinite(raw2)) return null;
  return raw2 / 100;
}
function detectEventType2(event) {
  const type = String(event.type ?? "");
  const obj = event.data?.object ?? void 0;
  switch (type) {
    ...
    case "invoice.payment_succeeded": {
      const reason = obj ? pick2(obj, "billing_reason") : void 0;
...
function extractCustomer(obj) {
  ...
  const contato = parsePhone2(phoneRaw);
  return { nome, email, contato };
}
function parseStripeWebhook(event) {
  const eventType = detectEventType2(event);
  if (!eventType) return null;
  const obj = event.data?.object;
  if (!obj) return null;
  const { nome, email, contato } = extractCustomer(obj);
  const valor = parseStripeAmount(
    pick2(obj, "amount_total", "amount_paid", "amount_due", "amount")
  ) ?? null;
  const gatewayCustomerId = pick2(obj, "customer", "customer_id") ?? null;
  const gatewayLastOrderId = pick2(obj, "id", "payment_intent", "subscription", "invoice") ?? null;
  const planoNome = pick2(
    obj,
    "metadata.plano_nome",
  ...
  return {
    ...
    planoNome,
    pixExpiraEm: null,
    // Stripe nao tem PIX expiration nativo no payload
    extras: {
      // Link util pra mensagens (cliente acessa fatura/checkout)
      hosted_invoice_url: pick2(obj, "hosted_invoice_url", "invoice_pdf") ?? "",
      checkout_url: pick2(obj, "url") ?? ""
    }
  };
}
function verifyStripeSignature(rawBody, headers) {
  ...
  const timestamp2 = parts.t?.[0];
  const v1Sigs = parts.v1 ?? [];
  if (!timestamp2 || v1Sigs.length === 0) {
    return { valid: false, reason: "invalid-signature-header" };
  }
  const tsAge = Math.abs(Math.floor(Date.now() / 1e3) - Number(timestamp2));
  if (!Number.isFinite(tsAge) || tsAge > 300) {
    return { valid: false, reason: "timestamp-too-old-or-invalid" };
  }
  const signedPayload = `${timestamp2}.${rawBody}`;
  const expected = crypto3.createHmac("sha256", secret).update(signedPayload).digest("hex");
  const expectedBuf = Buffer.from(expected, "utf8");
  for (const sig of v1Sigs) {
    const sigBuf = Buffer.from(sig, "utf8");
    if (sigBuf.length !== expectedBuf.length) continue;
    if (crypto3.timingSafeEqual(sigBuf, expectedBuf)) {
      return { valid: true };
    }
  }
...
function applyReplacements(raw2, ctx) {
  const replacements = {
    nome: ctx.lead.nome,
    primeiroNome: firstName(ctx.lead.nome),
    valor: ctx.lead.valorEstimado != null ? ctx.lead.valorEstimado.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL"
    }) : "",
    ...Object.fromEntries(
  ...
  return raw2.replace(/\{\{(\w+)\}\}/g, (_, key) => replacements[key] ?? "");
}
async function renderTemplate(template, ctx) {
...
// server/lib/flows.ts
async function loadFlow(event) {
  const rows = await db.select().from(flowSteps).where(and(eq(flowSteps.gatewayEvent, event), eq(flowSteps.ativo, true))).orderBy(asc(flowSteps.ordem));
  return rows.map((r) => ({
...
async function handleGatewayEvent(input) {
  ...
  for (const step of flow) {
    ...
    const agendadoPara = new Date(now.getTime() + step.delaySeconds * 1e3);
...
// server/lib/produtos.ts
async function findOrCreateProdutoByName(rawName) {
  ...
  const byMatch = await db.select().from(produtos).where(
    sql`EXISTS (
  ...
  return await db.query.produtos.findFirst({ where: eq(produtos.nome, nome) }) ?? null;
}
...
var URL2 = () => process.env.EVOLUTION_API_URL?.replace(/\/+$/, "");
var KEY = () => process.env.EVOLUTION_API_KEY;
var INSTANCE = () => process.env.EVOLUTION_INSTANCE_NAME;
...
async function sendText(numero, texto, opts) {
  try {
...
async function dispatchPending(now = /* @__PURE__ */ new Date()) {
  const pending = await db.select().from(mensagensAgendadas).where(
    and(
      eq(mensagensAgendadas.status, "pending"),
  ...
  for (const msg of pending) {
    ...
    if (!lead?.contato) {
      await db.update(mensagensAgendadas).set({ status: "skipped", erro: "Lead sem contato" }).where(eq(mensagensAgendadas.id, msg.id));
      skipped++;
      continue;
    }
    const result = await sendText(lead.contato, msg.conteudo);
    if (result.ok) {
      await db.update(mensagensAgendadas).set({
        status: "sent",
        enviadoEm: /* @__PURE__ */ new Date(),
      ...
      if (lead.status === "novo") {
        await db.update(leads).set({
          status: "contatado",
          primeiroContatoEm: /* @__PURE__ */ new Date(),
          atualizadoEm: /* @__PURE__ */ new Date()
        }).where(eq(leads.id, lead.id));
      }
    } else {
      await db.update(mensagensAgendadas).set({ status: "failed", erro: result.error }).where(eq(mensagensAgendadas.id, msg.id));
      failed++;
    }
  }
...
var produtosRoutes = new Hono2();
produtosRoutes.get("/", async (c) => {
  const list = await db.select().from(produtos).orderBy(asc(produtos.nome));
  return c.json(list);
});
...
function withCache(fn, keyPrefix, ttlSeconds) {
  return async (...args) => {
    ...
    cache.set(key, { value, expiresAt: now + ttlSeconds * 1e3 });
    return value;
  };
}
...
function produtoCondition(produtoId) {
  return produtoId == null ? [] : [eq(leads.produtoId, produtoId)];
}
async function getAllLeads(produtoId = null) {
  const cond = produtoCondition(produtoId);
  if (cond.length === 0) {
    return db.select().from(leads).orderBy(desc(leads.criadoEm));
  }
  return db.select().from(leads).where(and(...cond)).orderBy(desc(leads.criadoEm));
}
var getSidebarCounts = withCache(
  async () => {
    const [risco] = await db.select({ n: sql`count(*)::int` }).from(leads).where(inArray(leads.status, ["cliente_em_risco", "pix_gerado"]));
    const [fila] = await db.select({ n: sql`count(*)::int` }).from(mensagensAgendadas).where(eq(mensagensAgendadas.status, "pending"));
    return {
      emRisco: risco?.n ?? 0,
...
async function getDashboardMetrics(produtoId = null, since = null) {
  const cond = produtoCondition(produtoId);
  ...
  const pixPagosNoPeriodo = since ? pixPagos.filter((l) => l.pagouEm >= since) : pixPagos;
  ...
  const mrrPotencial = mrr + all.filter((l) => l.subscriptionStatus === "aguardando_pagamento").reduce((acc, l) => acc + (l.valorAssinatura ?? 0), 0);
  ...
  const ticketMedio = pixPagosNoPeriodo.length > 0 ? receitaTotal / pixPagosNoPeriodo.length : 0;
  const pendingMsgs = await db.select().from(mensagensAgendadas).where(eq(mensagensAgendadas.status, "pending"));
  const sentToday = await db.select().from(mensagensAgendadas).where(
    and(
...
async function getDailySeries(days = 30, produtoId = null) {
  const cond = produtoCondition(produtoId);
  const all = cond.length === 0 ? await db.select().from(leads) : await db.select().from(leads).where(and(...cond));
  const today = startOfDay(/* @__PURE__ */ new Date());
  const dailySeries = [];
...
async function getFaturamento(produtoId = null, since = null) {
  const conditions = [
...
async function getPlanoBreakdown(produtoId = null) {
  const cond = produtoCondition(produtoId);
  const all = cond.length === 0 ? await db.select().from(leads) : await db.select().from(leads).where(and(...cond));
  ...
  return Array.from(map.entries()).map(([plano, v]) => ({ plano, ...v })).sort((a, b2) => b2.total - a.total);
}

// server/routes/leads.ts
var leadsRoutes = new Hono2();
var leadSchema = external_exports.object({
  nome: external_exports.string().min(1, "Nome obrigat\xF3rio"),
  contato: external_exports.string().optional().nullable(),
  tipo: external_exports.enum(LEAD_TYPES),
  status: external_exports.enum(LEAD_STATUS).default("novo"),
  origem: external_exports.enum(LEAD_ORIGINS).optional().nullable(),
  valorEstimado: external_exports.union([external_exports.number(), external_exports.string()]).optional().nullable(),
  observacoes: external_exports.string().optional().nullable(),
  produtoId: external_exports.number().int().positive().optional().nullable()
});
...
function timestampsForStatus(status, now) {
  return {
    ...
    respondeuEm: status === "respondeu" || status === "convertido" || status === "reembolso_revertido" ? now : null,
    convertidoEm: status === "convertido" || status === "reembolso_revertido" ? now : null
  };
}
leadsRoutes.get("/", async (c) => {
  const produtoIdParam = c.req.query("produtoId");
  const produtoId = produtoIdParam && produtoIdParam !== "all" ? Number(produtoIdParam) || null : null;
...
leadsRoutes.post("/", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: parsed.error.issues[0]?.message ?? "Dados inv\xE1lidos" }, 400);
  }
...
leadsRoutes.put("/:id", async (c) => {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    return c.json({ error: "ID inv\xE1lido" }, 400);
  }
  const body = await c.req.json().catch(() => ({}));
  const parsed = leadSchema.safeParse(body);
  if (!parsed.success) {
  ...
  if (!prev) return c.json({ error: "Lead n\xE3o encontrado" }, 404);
  const updates = {
    nome: data.nome,
    contato: data.contato ?? null,
    tipo: data.tipo,
  ...
  if (data.status !== prev.status) {
    ...
    if ((data.status === "respondeu" || data.status === "convertido" || data.status === "reembolso_revertido") && !prev.respondeuEm) {
      updates.respondeuEm = now;
    }
    if ((data.status === "convertido" || data.status === "reembolso_revertido") && !prev.convertidoEm) {
      updates.convertidoEm = now;
    }
  }
...
leadsRoutes.patch("/:id/status", async (c) => {
  ...
  if (!status || !LEAD_STATUS.includes(status)) {
    return c.json({ error: "Status inv\xE1lido" }, 400);
  }
  const now = /* @__PURE__ */ new Date();
  const [prev] = await db.select().from(leads).where(eq(leads.id, id)).limit(1);
  if (!prev) return c.json({ error: "Lead n\xE3o encontrado" }, 404);
  const updates = { status, atualizadoEm: now };
  if (status !== "novo" && !prev.primeiroContatoEm) updates.primeiroContatoEm = now;
  if ((status === "respondeu" || status === "convertido" || status === "reembolso_revertido") && !prev.respondeuEm) {
    updates.respondeuEm = now;
  }
  if ((status === "convertido" || status === "reembolso_revertido") && !prev.convertidoEm) {
    updates.convertidoEm = now;
  }
  await db.update(leads).set(updates).where(eq(leads.id, id));
  invalidateCache("sidebar-counts");
...
var automacoesRoutes = new Hono2();
var DELAY_UNITS = {
  seconds: 1,
  minutes: 60,
  hours: 60 * 60,
...
function delayToSeconds(value, unit) {
  ...
  if (!multiplier) throw new Error("Unidade inv\xE1lida");
  if (!Number.isFinite(value) || value < 0) throw new Error("Delay inv\xE1lido");
  const s = Math.floor(value * multiplier);
  if (s > 60 * 60 * 24 * 30) throw new Error("Delay m\xE1ximo: 30 dias");
  return s;
}
function slugify(s, max = 40) {
  return s.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "").substring(0, max);
}
...
var createTemplateSchema = external_exports.object({
  ...
  descricao: external_exports.string().nullable().optional(),
  conteudo: external_exports.string().min(1).max(4e3),
  gatewayEvent: external_exports.string().optional()
});
automacoesRoutes.post("/templates", async (c) => {
  ...
  if (!key) return c.json({ error: "N\xE3o foi poss\xEDvel gerar key" }, 400);
  const existing = await db.query.messageTemplates.findFirst({
    where: eq(messageTemplates.key, key)
  });
  if (existing) return c.json({ error: `J\xE1 existe template com key "${key}"` }, 409);
  ...
  if (gatewayEvent) {
    const existingSteps = await db.select().from(flowSteps).where(eq(flowSteps.gatewayEvent, gatewayEvent));
...
automacoesRoutes.patch("/templates/:key", async (c) => {
  ...
  if (!conteudo) return c.json({ error: "Conte\xFAdo obrigat\xF3rio" }, 400);
  if (conteudo.length > 4e3) return c.json({ error: "Conte\xFAdo muito longo" }, 400);
  await db.update(messageTemplates).set({ conteudo, atualizadoEm: /* @__PURE__ */ new Date() }).where(eq(messageTemplates.key, key));
  return c.json({ ok: true });
});
automacoesRoutes.post("/templates/:key/revert", async (c) => {
  ...
  if (!tpl) return c.json({ error: `Template ${key} n\xE3o encontrado` }, 404);
  await db.update(messageTemplates).set({ conteudo: tpl.conteudoDefault, atualizadoEm: /* @__PURE__ */ new Date() }).where(eq(messageTemplates.key, key));
  return c.json({ ok: true });
});
automacoesRoutes.delete("/templates/:key", async (c) => {
  const key = c.req.param("key");
  const inUse = await db.select().from(flowSteps).where(eq(flowSteps.templateKey, key));
  if (inUse.length > 0) {
    return c.json(
      { error: `N\xE3o d\xE1 pra apagar \u2014 template est\xE1 em ${inUse.length} passo(s).` },
      409
    );
  }
...
var addStepSchema = external_exports.object({
  templateKey: external_exports.string().min(1),
  gatewayEvent: external_exports.string().min(1),
  delayValue: external_exports.number(),
  delayUnit: external_exports.string().default("hours")
});
automacoesRoutes.post("/flow-steps", async (c) => {
  ...
  try {
    ...
    return c.json({ error: e instanceof Error ? e.message : "Delay inv\xE1lido" }, 400);
  }
  const existing = await db.select().from(flowSteps).where(eq(flowSteps.gatewayEvent, gatewayEvent));
  if (existing.length >= 5) {
    return c.json({ error: "M\xE1ximo de 5 passos por fluxo" }, 409);
  }
...
automacoesRoutes.patch("/flow-steps/:id", async (c) => {
  const id = Number(c.req.param("id"));
  if (!Number.isInteger(id) || id <= 0) return c.json({ error: "ID inv\xE1lido" }, 400);
  const body = await c.req.json().catch(() => ({}));
  ...
  await db.update(flowSteps).set({ delaySeconds, ativo, cancelPrevious, atualizadoEm: /* @__PURE__ */ new Date() }).where(eq(flowSteps.id, id));
  return c.json({ ok: true });
});
automacoesRoutes.delete("/flow-steps/:id", async (c) => {
  ...
  if (!step) return c.json({ error: "Passo n\xE3o encontrado" }, 404);
  await db.delete(flowSteps).where(eq(flowSteps.id, id));
  const stillInUse = await db.select().from(flowSteps).where(eq(flowSteps.templateKey, step.templateKey));
...
automacoesRoutes.post("/messages", async (c) => {
  ...
  const existing = await db.select().from(flowSteps).where(eq(flowSteps.gatewayEvent, gatewayEvent));
  if (existing.length >= 5) return c.json({ error: "M\xE1ximo de 5 mensagens por evento" }, 409);
  const baseKey = slugify(nome, 30);
  if (!baseKey) return c.json({ error: "N\xE3o foi poss\xEDvel gerar key" }, 400);
  let key = `${gatewayEvent}_${baseKey}`;
  let suffix = 1;
  while (await db.query.messageTemplates.findFirst({
    where: eq(messageTemplates.key, key)
  })) {
    suffix++;
    key = `${gatewayEvent}_${baseKey}_${suffix}`;
    if (suffix > 50) return c.json({ error: "N\xE3o foi poss\xEDvel gerar key \xFAnica" }, 500);
  }
  await db.insert(messageTemplates).values({
    key,
    nome,
...
automacoesRoutes.delete("/eventos/:gatewayEvent", async (c) => {
  ...
  if (!gatewayEvent) return c.json({ error: "Evento obrigat\xF3rio" }, 400);
  const steps = await db.select().from(flowSteps).where(eq(flowSteps.gatewayEvent, gatewayEvent));
  ...
  for (const s of steps) {
    ...
    const usedElsewhere = await db.select().from(flowSteps).where(eq(flowSteps.templateKey, s.templateKey));
